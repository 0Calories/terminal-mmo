...
import { parseTerrain, stepEntity, spawnPlayer, BOX } from "../src"
...
test("gravity: an entity falls and lands on the ground", () => {
  const e = settle(FLAT, spawnPlayer(1, 2, 0))
  expect(e.onGround).toBe(true)
  expect(e.vy).toBe(0)
})

test("walls: horizontal movement is blocked by a solid column", () => {
  const e = settle(WALL, spawnPlayer(1, 1, 0), { moveX: 1, jump: false })
...
test("jump: leaves the ground with upward velocity", () => {
  const grounded = settle(FLAT, spawnPlayer(1, 2, 0))
