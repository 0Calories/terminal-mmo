...
function serverAvatar(
	...
	x: number,
	handle = 'hero',
	// Level gates the capability verbs: default 1 (attack only); block/dodge/skill cases
	// raise it past the relevant unlock (ADR 0024 §5).
	level = 1,
): ServerAvatar {
	return {
		sessionId,
...
// The swing hitbox is live only in the active window, so a hit never lands on the tick
// `attack` is pressed. The outcome tests below care about a connect's consequences, not
// wind-up timing, so they prime the swing straight into its active phase (ADR 0017 §1).
...
// A chaser parked mid-active so its hitbox is live this tick, to the LEFT of an Avatar
// at x=20 facing into it. attackT > 0 suppresses a re-commit, so it lands exactly one
// active-phase strike (ADR 0017 §9).
...
test('stepZone keeps the melee hitbox live ONLY during the swing active phase', () => {
	// A real swing driven tick by tick (no priming), unlike the primed outcome tests.
	const m = spawnMonster('chaser', 2, 20 + BOX.w, y);
...
test('a skill intent damages a Monster and the server folds its cooldown + log', () => {
	// Detailed skill gating lives at the resolveCombat seam (combat.test.ts); here we
	// only confirm a skill intent flows through a full stepZone tick.
	const m = spawnMonster('chaser', 2, 20 + BOX.w, y);
...
test('the Avatar landing the killing blow earns the XP and, standing on the kill, collects its instanced loot', () => {
	...
	const av = primeSwing(serverAvatar(7, 20));
	// The Dungeon always drops (ADR 0024 §2), so the roll is deterministic; the killer
	// stands on the kill site and grabs its own private Drop the same tick.
...
test('a far contributor leaves its instanced Drop resting, then collects it on touch', () => {
	// The killer stands on the kill (auto-grabs); the far helper's private Drop rests
	// until it walks over — collect-on-touch (#238).
	const m = spawnMonster('chaser', 2, 20 + BOX.w, y);
	m.hp = 4;
	m.contributors = [7, 8];
...
test('overlapping a Monster deals NO contact damage (passive contact damage removed)', () => {
	// A chaser stacked on the Avatar, parked in a fake recovery so it never commits:
	// pure overlap, zero damage.
...
test('a melee committer commits a telegraphed swing and damages ONLY in its active phase', () => {
	// Chaser just inside melee range of a stationary Avatar.
	...
	// First tick: it commits into wind-up — the telegraph, no damage yet.
...
test('a committer cannot re-attack during its recovery — a punishable opening', () => {
	// Find the recovery window: attackT still > 0 (committed) yet phase recovery — no
	// damage, swing not restarted, so the Player can punish.
	const m = spawnMonster('chaser', 2, 20 + MONSTER.meleeRange, y);
	m.onGround = true;
	const av = serverAvatar(7, 20);
	let state: ZoneState = { zone: zoneWith([m]), avatars: [av], tick: 0 };
	...
	for (let i = 0; i < 40; i++) {
		...
		if (swingPhase(mon.attackT) === 'recovery') {
			sawRecovery = true;
			// During recovery the committer is exposed: a punisher's swing connects and
			// doesn't cancel the recovery.
...
test('a committer in its active phase can Stagger a poise-broken Avatar (full hit-reaction payload)', () => {
	// Poise pre-broken low, so the active hit Staggers: full hit-reaction payload
	// (Hitstun + Knockback).
	const m = spawnMonster('chaser', 2, 20 + MONSTER.meleeRange, y);
	m.onGround = true;
	const av = serverAvatar(7, 20);
...
test('the heavy brute commits a telegraphed swing and damages ONLY in its active phase, for its heavy hit', () => {
	// Same commit shape as the chaser, but the connect deals the brute's heavier damage.
	...
	// First tick: it commits into wind-up, no damage yet.
	state = stepZone(state, [holdAt(7, av.avatar)], 16);
	const snap = snapshotFor(state, 7);
	expect(snap.monsters[0].action.move).toBe('basic');
	expect(snap.monsters[0].action.phase).toBe('windup');
...
test('the brute is a poise-tank: it spawns with a much larger Poise pool than the default', () => {
	// High-poise identity: it shrugs off a flurry a chaser would break under.
...
test('the brute attacks deliberately: a commit cool-down keeps it from re-swinging the instant it recovers', () => {
	// Unlike the chaser (commitCd 0), the brute waits out a cool-down after a swing — a
	// punishable opening.
	const m = spawnMonster('brute', 2, 20 + BRUTE.meleeRange, y);
	m.onGround = true;
	const av = serverAvatar(7, 20);
	...
	// The cadence is a pause, not a wedge: once the cool-down drains, it commits again.
...
test('a Dodge negates a Monster strike during its i-frame active window', () => {
	// A committer strikes an Avatar mid-Dodge (active window), so the hit is negated.
...
test('a Dodge in its recovery window does NOT grant i-frames — the hit connects', () => {
	// The Avatar's Dodge has decayed into recovery — exposed, so the strike lands.
	const m = strikingCommitterAt20();
	const av = serverAvatar(7, 20);
...
test('a Dodge slips a projectile during its active window but not its recovery', () => {
	// The same i-frame gate covers ranged hits.
...
test('a dodge intent loads the i-frame timer through stepZone (active on the first tick)', () => {
	...
	// The report carries the client's already-gated `dodge` decision; the server loads
	// the i-frame timer on trust.
...
//
// strikingCommitterAt20 strikes from the LEFT of an Avatar at x=20, so a frontal Guard
// needs facing -1; facing +1 is a rear hit Guard ignores.
...
test('an unguarded committer chip emits source-less hurt-blood biased away from the Monster', () => {
	// Incoming hurt is never predicted: the chip resolves to a source-less `hit`, so the
	// snapshot delivers the blood to the victim too. Biased away from the committer at
	// x=16 (→ dir +1) (ADR 0013 §3).
	const m = strikingCommitterAt20();
...
test('an uncollected Drop fades once its ttl drains', () => {
	// A Drop far from its owner with almost no ttl: one tick drains it away (#238).
...
test('clientStepAvatar predicts platformer movement and decays the hurt timer', () => {
	// attackT decay now lives in resolveCombat; clientStepAvatar only advances physics
	// and decays hurtT.
	...
	expect(predicted.attackT).toBe(0.3); // no longer decayed here
});
...
test('a ranged poker telegraphs through stepZone: it commits a swing and fires only on the active frame', () => {
	// A shooter commits the swing but does NOT fire on the commit tick — the shot appears
	// only when it crosses into active (no auto-fire, ADR 0017 §8).
...
test('an unguarded heavy projectile Staggers the Avatar on a Poise break, like a melee hit', () => {
	// A shot is a first-class hit: its poise damage can break the pool → Hitstun +
	// Knockback, the same path a melee connect uses (ADR 0017 §8).
	const av = serverAvatar(7, 20);
	const pr = makeProjectile({
		x: 22,
		y: av.avatar.y + 2,
...
test('Swat: a live active melee frame DESTROYS a hostile shot', () => {
	...
	// The swat is a LIGHT impact clink: intensity = the shot's own damage, NO poise.max
	// bump (unlike a break), and source-less (ADR 0019).
...
test('Hitstun locks control but not physics: a staggered Monster flies under Knockback, not chasing', () => {
	...
	// A chaser's AI would home LEFT, but Hitstun suppresses it — the body drifts RIGHT
	// under Knockback instead.
	const a = state.avatars[0].avatar;
...
test('automatic post-hit i-frames are gone: a staggered Monster takes a second hit', () => {
	...
	expect(state.zone.monsters[0].stunT ?? 0).toBeGreaterThan(0);
	// A fresh swing clears the per-swing registry, so a second swing lands on the still-
	// staggered Monster — the automatic post-hit i-frame that used to block it is gone.
...
test('a default chaser Poise-breaks strictly before it dies (the break is observable)', () => {
	// A tuning guard: if a chaser broke only on the hit that killed it, the Stagger would
	// never be seen. Pure accumulation, so hit counts come from the constants.
...
test('sustained real swings Stagger a chaser while it is still alive (regen is gated)', () => {
	// Genuine swings tick by tick: the regen delay must hold the pool down across swings
	// so a break lands before the kill — an always-on regen would refill between swings
	// and never Stagger.
	const m = spawnMonster('chaser', 2, 20 + BOX.w, y);
	let state: ZoneState = {
		zone: zoneWith([m]),
...
test('Poise regenerates once pressure stops (a spaced poke does not accumulate to a break)', () => {
	// One chip, then idle: the regen-delay drains and the pool refills, so a single chip
	// never compounds into a break.
	const m = spawnMonster('chaser', 2, 20 + BOX.w, y);
...
test('a landed swing deals the equipped weapon damage but the SHARED poise chip', () => {
	// The weapon contributes damage only; the Poise chip is the one COMBAT constant, so
	// no weapon staggers faster from data.
	const m = spawnMonster('chaser', 2, 20 + BOX.w, y);
	m.hp = 100;
	const av = primeSwing(serverAvatar(7, 20));
...
test('every weapon index swings the one shared moveset — timing from COMBAT.swing', () => {
	// Whatever the weapon index (default, or an out-of-range one that clamps), the shared
	// phase total makes a mid-active swing replicate as `active` — no per-weapon timing.
...
// Consumes an already-decided death set: monsters pay out fully zone-local (XP + instanced
// Drops + respawn), avatars emit only the died-this-tick set + an in-place respawn.
...
test('resolveDeaths reports the died-this-tick set and defers the respawn to the caller', () => {
	...
	// The pass REPORTS the death but does NOT respawn (that is the caller's job, after loot
	// collection), so the fallen Avatar is left unmoved here.
